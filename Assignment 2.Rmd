...
where $\text{I}(\cdot)$ equals $1$ if $U \leq F(X)$ and $0$ otherwise.
...
The indicator function constructs a test that compares a logistic regression function and uniform random variable.

The logistic function returns a sample drawn from the uniform random variable $X$. 

The response is compared to an independent sample of random variable from the uniform distribution, $U$. 

Where the logistic response is less than or equal to the uniform random variable, the indicator functions maps $Y$ to a value 1, and 0 otherwise. 
...
The figure above plots the distribution of test error rates observed from the bootstrap simulation of 10 classifying models.

All models show a negative skew in the distribution of test error rates (TER), while the linear probability model (`lm`) shows the greatest range of TER variance that is observered in the models.
